import {
  ...
  import {useMutationCreateVinculo} from "service/vinculos"
  
  const VinculosModalCreate = ({ isOpen, onClose }) => {
    const {
      register,
      handleSubmit,
      control,
    ...
    const { mutate, isLoading } = useMutationCreateVinculo({
      onError: ({ response }) => {
      ...
      onSuccess: async () => {
        ...
        await queryClient.refetchQueries(["queryListVinculo"]);
    ...
    return (
      <Modal isOpen={isOpen} onClose={onClose} size="3xl">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Cadastro de Vínculo - Instituição / Programa de Ensino</ModalHeader>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <Flex direction="column" my="30px">
                <FormControl isInvalid={errors.nome}>
                  <FormLabel htmlFor="nome">Orienta</FormLabel>
                  <Input
                    id="nome"
                    type="text"
  ...
  export default VinculosModalCreate;